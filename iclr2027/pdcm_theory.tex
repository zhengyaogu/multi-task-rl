...
\end{equation}
Divide by $\alpha$ and let $\alpha\downarrow 0$. Differentiability yields \eqref{eq:app_tangent_bound}.
\end{proof}
...
\end{equation}
\eqref{eq:app_safety_certificate} is equivalently
\begin{equation}
    c_t(p_t)\le h_t(p_t)
...
Suppose Assumptions~\ref{ass:app_concavity}--\ref{ass:app_bounded} hold and $(p_t,\lambda_t)$ is generated by \eqref{eq:app_primal_update}--\eqref{eq:app_dual_update}. Then for every horizon $T\ge 1$, every $p\in\mathcal{P}_T$, and every $\eta_p,\eta_\lambda>0$,
\begin{equation}
    R_T(p)
    \le
    \frac{\log K}{\eta_p}
...
\begin{proof}
We first prove a one-step entropic mirror-ascent inequality. By optimality of \eqref{eq:app_primal_update} and the three-point identity for the negative-entropy Bregman divergence, for every $p\in\Delta_K$,
\begin{equation}
...
\end{equation}
Summing over $t$, using $p_1=u$, $D_{\mathrm{KL}}(p\|u)\le\log K$, and \eqref{eq:app_basic_bounds}, gives
\begin{equation}
...
We now relate the score to utility regret and the safety residuals. From \eqref{eq:app_affine_residual},
\begin{equation}
    h_t(p_t)-h_t(p)=G_t(p-p_t).
    \label{eq:app_h_difference}
\end{equation}
Therefore, using \eqref{eq:app_score},
\begin{align}
    \langle s_t,p-p_t\rangle
    &=
...
\end{equation}
Combining \eqref{eq:app_mirror_sum}, \eqref{eq:app_safe_drop}, and \eqref{eq:app_F_concavity} yields
\begin{equation}
    R_T(p)
    +
...
It remains to control the dual term. Projection onto the nonnegative orthant is nonexpansive and fixes the origin, hence \eqref{eq:app_dual_update} gives
\begin{align}
    \|\lambda_{t+1}\|_2^2
    &\le
...
\end{equation}
Substituting \eqref{eq:app_dual_inner_bound} into \eqref{eq:app_predual} proves \eqref{eq:app_finite_regret_bound}.
...
\begin{equation}
    ...
    \Lambda_T.
\end{equation}
Taking positive parts proves the second inequality in \eqref{eq:app_constraint_bound}. The first follows from \eqref{eq:app_true_vs_linearized} after summing over $t$. Finally, if $\Lambda_T\le\Lambda$ and both step sizes are of order $T^{-1/2}$, every term in \eqref{eq:app_finite_regret_bound} and \eqref{eq:app_constraint_bound} is $O(\sqrt{T})$.
\end{proof}
...
\end{equation}
Since $F$ is concave and each constraint function $U_k(u)-U_k(p)-\epsilon_k$ is convex, \eqref{eq:app_stationary_problem} is a convex optimization problem after negating the objective.

\begin{theorem}[Dual-weighted marginal-utility matching]
\label{thm:app_matching}
Assume \eqref{eq:app_stationary_problem} satisfies Slater's condition, and let $(p^\star,\lambda^\star)$ be a primal-dual optimum. Define
\begin{equation}
    s^\star
    \coloneqq
...
\begin{proof}
Rewrite \eqref{eq:app_stationary_problem} as the convex minimization problem
\begin{equation}
\begin{aligned}
...
\end{equation}
Hence $s_j^\star=\mu^\star-\alpha_j^\star$. If $p_j^\star>0$, complementary slackness for $-p_j\le0$ gives $\alpha_j^\star=0$, so $s_j^\star=\mu^\star$. If $p_j^\star=0$, then $\alpha_j^\star\ge0$, so $s_j^\star\le\mu^\star$. The remaining KKT conditions give \eqref{eq:app_complementarity}.
