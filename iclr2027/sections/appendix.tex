...
We selected four heterogeneous tasks served from the NeMo-Gym servers, some requiring tool use or multi-turn capabilities and others not.
...
\label{app:hyper}
Table~\ref{tab:pdcm-hparams} reports the hyperparameters for our two \textsc{PDCM} runs: the mix4
(IF-split) run with the full-Jacobian sampler and the aczpwm run with the scalar sampler over difficulty
tiers. Both use Qwen3-4B with GSPO, learning rate $10^{-6}$, $n{=}8$ rollouts per prompt, and $600$
steps; they differ mainly in the curvature object (full $K\!\times\!K$ Jacobian vs.\ per-arm scalar),
the number of controller arms ($K{=}4$ domains vs.\ $K{=}16$ difficulty tiers), and the sequence and
batch budget of the two task suites. Full settings are given in the table.


% Hyperparameter table for two PDCM runs. Requires \usepackage{booktabs}.
% Col A: mix4-ifsplit PDCM (full-Jacobian sampler); Col B: aczpwm PDCM (scalar sampler, difficulty-tier fold).
\begin{table}[t]
\centering
\small
\caption{Hyperparameters for the two \textsc{PDCM} runs: the mix4 (IF-split) run with the full-Jacobian
sampler, and the Reasoning-Gym run with the scalar sampler over difficulty tiers. Values are shared
unless noted; the two differ mainly in the curvature object (full $K\times K$ Jacobian vs.\ per-arm scalar),
the fold granularity ($K$), and the sequence/batch budget of the two task suites.}
\label{tab:pdcm-hparams}
\begin{tabular}{l l l}
\toprule
Hyperparameter & Nemo-Gym (Jacobian) & Reasoning-Gym \\
\midrule
Primal step $\eta_p$      & 0.1               & 0.3 \\
Dual step $\eta_\lambda$        & 0.01              & 0.05 \\
Window $H$                & 10                & 10 \\
Dither $\gamma$ (init/floor) & 0.20 / 0.10    & 0.20 / 0.10 \\
Dither anneal steps       & 400               & 400 \\
Lag budget $\varepsilon$  & 0.1               & 0.1 \\
Warmup steps              & 20                & 40 \\
\addlinespace
...
Total training steps      & 600               & 600 \\
\addlinespace
\multicolumn{3}{l}{\emph{Sequence / rollout}} \\
Max prompt / response len & 8192 / 8192       & 2048 / 4096 \\
vLLM max-model-len        & 24576             & 6144 \\
\bottomrule
\end{tabular}
\end{table}
\newpage
...
simpler variant keeps only the diagonal. The restriction amounts to assuming that each task's improvement depends on the allocation only
...
diagonal form used by \Cref{thm:pdcm_no_regret}.
