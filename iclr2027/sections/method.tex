...
As we discussed in \Cref{sec:bg_allocation}, naively maximizing the total policy improvement in \Cref{eq:bg_telescope} can lead to skewed development
...
G_t
    ...
    \end{bmatrix}
\end{equation}
Here, $G_t$ represents the Jacobian of the utility functions with respect to the allocation. We discuss a variant of PDCM where $G_t$ is diagonal in Appendix~\ref{app:diagonal}. 
...
O(\sqrt{T}).
\end{equation}
Consequently, both the average regret and the average excess counterfactual lag vanish at rate $O(T^{-1/2})$. We provide a proof of in Appendix~\ref{app:theory}.
\end{theorem}
