...
\end{equation}
\Cref{eq:constraint_uniform_baseline} marks a departure from the parametric constraint used in previous work~\citep{chen2025aioli,ye2025datamixinglawsoptimizing,kang2025autoscalescaleawaredatamixing}, by enforcing an upper-limit on the step-wise allocation deficit. In practice, estimating $U_{t, k}(u)$ requires a parallel run with uniform allocation $u$, which is cost-prohibitive. We relax the parametric assumptions in recent work~\citep{ye2025datamixinglawsoptimizing,chen2025aioli,kang2025autoscalescaleawaredatamixing} to one in which the policy-improvement surface with respect to mixture allocation is approximately concave along linear interpolation paths. 
