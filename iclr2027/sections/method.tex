...
We add the following constraint that prevents any task from falling behind
...
\end{equation}
\Cref{eq:constraint_uniform_baseline} marks a departure from the parametric constraint used in previous work~\citep{chen2025aioli,ye2025datamixinglawsoptimizing,kang2025autoscalescaleawaredatamixing}, by enforcing an upper-limit on the step-wise allocation deficit. In practice, it is cost-prohibitive to estimate 
$U_{t, k}\left( u \right)$, as it involves computing the
counterfactual policy improvement of a uniform baseline. We relax the parametric assumptions in recent work~\citep{ye2025datamixinglawsoptimizing,chen2025aioli,kang2025autoscalescaleawaredatamixing} to one in which the policy-improvement surface with respect to mixture allocation is approximately concave along linear interpolation paths. 
