...
methods. \citep{unitary2022kurin} found no clear advantage of the aforementioned methods against PCGrad, so we do not expect further variants to
...
The closest prior formulation, Aioli~\citep{chen2025aioli}, casts pre-training data mixing as the same kind of optimization problem but closes it with a power mixing law tailored to pre-training dynamics (\Cref{sec:related_work}). PDCM assumes only directional concavity of the task-improvement and utilizes local first-order information without over-prescribing the general shape of the learning curve.
